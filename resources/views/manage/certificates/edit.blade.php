...
<h1 class="page-title">Edit Certificate</h1>
@stop

@section('content')

<form method="POST" action="{{route('certificates.update',$certificate->id)}}" accept-charset="UTF-8"
				...
				<div class="box-body">
					...
					<div class="form-group">
						...
						<select class="form-control" name="student_id" id="student_id">
							...
							<option value="{{$student->id}}"  {{($student->id == $certificate->student_id)?'selected':''}}>{{$student->name}}</option>
							@endforeach
						</select>
					</div>
					<div class="form-group">
						<label for="multi_choice">Multi-Choise Point</label>
						<input type="number" id="multi_choice" name="multi_choice"  class="form-control" data-parsley-type="number" data-parsley-required="true" value="{{$certificate->multi_choice}}"></input>
					</div>
					<div class="form-group">
						<label for="practice">Practice Point</label>
						<input type="number" id="practice" name="practice"  class="form-control" data-parsley-type="number" data-parsley-required="true" value="{{$certificate->practice}}"></input>
					</div>
					<div class="form-group">
						<label for="cer_code">Certificate Code</label>
						<input type="text" id="cer_code" name="cer_code"  class="form-control" data-parsley-type="number" data-parsley-required="true" value="{{$certificate->cer_code}}"></input>
					</div>
					<div class="form-group">
						<label for="issue_code">Issue Code</label>
						<input type="text" id="issue_code" name="issue_code"  class="form-control" data-parsley-type="number" data-parsley-required="true" value="{{$certificate->issue_code}}"></input>
					</div>
					<div class="form-group">
